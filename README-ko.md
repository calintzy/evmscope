...
AI 에이전트를 위한 EVM 블록체인 인텔리전스 툴킷. 토큰 가격, 가스비 비교, 스왑 견적, DeFi 수익률, 허니팟 탐지, 브릿지 경로, TX 시뮬레이션, NFT 조회, 거버넌스 프로포절 등 23개 도구를 제공하는 단일 MCP 서버.

> "AgentKit으로 실행하고, evmscope로 판단하세요."

## 특징

- **23개 도구** — 가격, 가스비 비교, 스왑 견적, DeFi 수익률, 허니팟 탐지, 브릿지 경로, TX 시뮬레이션, 이벤트 로그, 토큰 홀더, 승인 상태, TVL, 고래 추적, 잔고, 토큰 정보, ENS, TX 상태, TX 해석, ABI 조회, 주소 식별, NFT 정보, NFT 메타데이터, 거버넌스 프로포절
...
7개 EVM 체인의 가스비를 한 번에 비교합니다. 최저가 순 정렬.

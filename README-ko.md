...
Claude, GPT 같은 AI 에이전트는 실시간 블록체인 데이터에 접근할 수 없습니다. "ETH 가격이 얼마야?", "이 지갑 잔액 알려줘"라고 물어보면 "실시간 데이터에 접근할 수 없습니다"라는 답을 받게 됩니다.

evmscope는 MCP 프로토콜을 통해 AI 에이전트에게 26개 온체인 도구를 직접 제공합니다 — 토큰 가격, 지갑 잔액, DeFi 수익률, 고래 추적, 허니팟 탐지 등. API 키 없이, 설정 없이, 연결하면 바로 동작합니다.

## 누구를 위한 도구인가?

| 사용자 | 활용 방식 |
|--------|----------|
| **AI 에이전트 개발자** | MCP 서버로 연결하여 AI에게 온체인 분석 능력 부여 |
| **크립토 트레이더 & 리서처** | 터미널에서 토큰, 지갑, 프로토콜을 직접 조회 |
| **DeFi 사용자** | 안전 도구 — 허니팟 탐지, 승인 상태 확인, 고래 추적 |
...
현재 트랜잭션 수수료를 slow/normal/fast 3단계로 USD 예상 비용과 함께 조회합니다.
...
7개 EVM 체인의 트랜잭션 수수료를 한 번에 비교합니다. 최저가 순 정렬.
...
DEX 스왑 견적을 조회합니다 (ParaSwap 기반, 최적 경로, 트랜잭션 수수료 포함, ETH→WETH 자동 치환).
...
트랜잭션을 시뮬레이션합니다 (eth_call + estimateGas, 트랜잭션 수수료 USD 환산, revert reason 디코딩).
...
지갑의 ERC-721 NFT 보유 수량 및 토큰 목록을 조회합니다.
...
특정 NFT 토큰의 메타데이터(이름, 이미지, 속성)를 조회합니다.
...
Snapshot 기반 거버넌스 프로포절을 조회합니다 (active/closed/all).
...
| `EVMSCOPE_RPC_URL_AVALANCHE` | Avalanche 전용 RPC 엔드포인트 | RPC_URL 폴백 |
| `EVMSCOPE_RPC_URL_BSC` | BSC 전용 RPC 엔드포인트 | RPC_URL 폴백 |
...
| `EVMSCOPE_SNOWTRACE_KEY` | Snowtrace API 키 (Avalanche) | ETHERSCAN_KEY 폴백 |
| `EVMSCOPE_BSCSCAN_KEY` | BscScan API 키 (BSC) | ETHERSCAN_KEY 폴백 |
